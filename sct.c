...
//   Or:
//     cc -std=c99 -O2 -I /usr/X11R6/include -o $HOME/sct sct.c \
//       -L /usr/X11R6/lib -lm -lX11 -lXrandr
// Public domain, do as you wish
//
// Bash usage (add to .bashrc), requires yad:
//     export SCT
//     sct(){
//       SCT=$(yad --title "Display tint" --scale --value=${SCT:=6300} --min-value=1000 --max-value=10000)
//       [[ $SCT ]] && $HOME/sct $SCT
//     }
...
int main(int argc, char **argv) {
	...
	if (argc == 1) return 0;
	temp = atoi(argv[1]);
	if (temp < 1000 || temp > 10000) temp = 6500;

	temp -= 1000;
	double ratio = temp % 500 / 500.0;
#define AVG(t) whitepoints[temp / 500].t * (1 - ratio) + whitepoints[temp / 500 + 1].t * ratio
	double gammar = AVG(r);
	double gammag = AVG(g);
	double gammab = AVG(b);
